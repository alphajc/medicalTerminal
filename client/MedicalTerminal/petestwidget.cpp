...
PETestWidget::PETestWidget(QWidget *parent) :
    QWidget(parent)
{
    this->initVariables();
}

void PETestWidget::clearVector()
{
    m_dataVec.clear();
}
void PETestWidget::initVariables()
{
...
void PETestWidget::paintEvent(QPaintEvent*)
{
    QPainter painter;
    painter.begin(this);
...
void PETestWidget::resetVariables(QPainter*)
{
...
void PETestWidget::drawBackground(QPainter *painter)
{
    painter->save();
...
void PETestWidget::drawBox(QPainter *painter)
{
    painter->save();
    QPointF topLeftPot(m_leftSpace*1.5,m_topSpace);
...
void PETestWidget::drawText(QPainter *painter)
{
    painter->save();
...
void PETestWidget::drawGraph(QPainter *painter)
{
    painter->save();
    ...
    {
        qreal x=startX;
        qreal y=m_boxRect.bottomRight().y()-(qreal)m_dataVec.at(i)*(m_boxRect.height()/200);
        QPointF dataPot(x,y);
    ...
    painter->drawPolyline(potVec);

    // draw points
    if(m_bShowPoint)
    {
        painter->setOpacity(0.5);
        for(int i=0;i<potVec.count();i++)
        {
...
void PETestWidget::drawTitle(QPainter *painter)
{
    painter->save();
...
void PETestWidget::addData(qreal data)
{
    m_dataVec.push_front(data);
    updateVector();
}

void PETestWidget::updateVector()
{
